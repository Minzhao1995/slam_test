...
struct FRAME
{
    ...
    cv::Mat left, right; //该帧对应的彩色图与深度图
    cv::Mat desp;       //特征描述子
    vector<cv::KeyPoint> kp; //关键点
    Eigen::Isometry3d T_lastKF;//transform to last keyframe
    Eigen::Isometry3d T_global;//transform to last keyframe
};
...
private:
     ...
     vector< FRAME > keyframes;    //keyframes
          vector< FRAME > keyframes2;    //keyframes
    int currIndex;
    int startIndex;
    int endIndex;
    FRAME currFrame;
